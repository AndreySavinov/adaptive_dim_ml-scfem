...
if G_rhs{5} >= 0
    ...
    elseif G_rhs{5} == 2
        ...
        Goal_unc_ref = u_col_point_ref*G_ref*u_col_point_ref.';
    elseif G_rhs{5} == 3
        [~, wz_ref] = goafem_stochcol_fem_setup(paras_fem_ref{1}, paras_fem_ref{2}, a_unit, G_rhs);
        u_col_point_ref = wz_ref.'*sols_u_ref;
...
l7 = min(l5, min(GoalErr2(1:iterno)));
l8 = max(l6, max(GoalErr2(1:iterno)));
...
figure(15);
p2 = loglog(...
       [10^-1 10^8], (0.08)*[10^-1 10^8].^(-1/2), 'k--');%((c3+c4)/4)*
hold on
...
% straight_line = (0.08)*dof_iterno.^(-1/2);
% stochcol_data2pgfplots;